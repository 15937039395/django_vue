...
class TreeListView(View):
    ...
    def buildTreeMenu(self, sysMenuList):
        ...
        import logging
        logging.info(f'菜单树结果: {resultMenuList}')
        return resultMenuList

    def get(self, request):
        menuQuerySet = SysMenu.objects.order_by("order_num")
        import logging
        logging.info(f'菜单查询集: {menuQuerySet}')
        # 构造菜单树
    ...
    def post(self, request):
        ...
        if data['id'] == -1:  # 添加
            obj_sysMenu = SysMenu(name=data['name'], icon=data['icon'],
                                  ...
                                  remark=data['remark'])
            obj_sysMenu.create_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            obj_sysMenu.save()
        else:  # 修改
            ...
            obj_sysMenu.update_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            obj_sysMenu.save()
