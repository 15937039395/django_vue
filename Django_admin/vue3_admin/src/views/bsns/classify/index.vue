...
﻿<template>
  <div class="app-container">
    <el-row :gutter="20" class="header">
      <el-col :span="7">
        <el-input placeholder="请输入名称" v-model="queryForm.query" clearable></el-input>
      </el-col>
      <el-button type="primary" :icon="Search" @click="initClassifyList">搜索</el-button>
    ...
    >
      ...
      <el-table-column  prop="id" label="分类ID" sortable width="300"/>

      <el-table-column  prop="project_number" label="分类名称" sortable width="750"/>
    ...
    <Dialog v-model="dialogVisible" :dialogVisible="dialogVisible" :id="id" :dialogTitle="dialogTitle"
            @initClassifyList="initClassifyList"></Dialog>
  </div>
</template>
...
const handleDelete = async (id) => {
  ...
  const res = await requestUtil.del("classify/action", ids)
...
const initClassifyList=async ()=>{
  ...
  const res=await requestUtil.post("classify/search",queryForm.value)
  tableData.value=res.data.classify_List
