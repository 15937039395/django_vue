...
﻿<template>
  <div class="app-container">
    <el-page-header @back="goBack" content="新增知识库" />
...
// 监听表单内容变化，用于检测富文本编辑器的变化
// 注释掉监听器，因为不再需要检测未保存更改
// watch(form, () => {
//   // 这个watcher会在表单任何字段变化时触发
//   // 用于确保富文本编辑器内容变化能被检测到
// }, { deep: true });
...
const hasUnsavedChanges = () => {
  // 检查是否有填写内容
  return form.value.title || form.value.content || form.value.remark;
};
...
onMounted(() => {
  // 默认填充当前用户信息
  form.value.dept_id = Number(currentUser.dept_id) || 0;
  form.value.user_id = Number(currentUser.id) || 0;
  loadOptionData();
});
</script>
