...
﻿<template>
  ...
  <div class="app-container">
    <el-row :gutter="20" class="header">
      <el-col :span="7">
        <el-input placeholder="请输入职级名..." v-model="queryForm.query" clearable></el-input>
      </el-col>
      <el-button type="primary" :icon="Search" @click="initLevelList">搜索</el-button>
    ...
    <el-table
      ...
      <el-table-column  prop="name" label="职级名称" min-width="200"/>
...
<!--      <el-table-column  prop="update_time" label="更新时间" width="180"/>-->
    ...
    <Dialog v-model="dialogVisible" :dialogVisible="dialogVisible" :id="id" :dialogTitle="dialogTitle"
          @initLevelList="initLevelList"></Dialog>
  </div>
</template>
...
const handleDialogValue = (roleId) => {
  if (roleId) {
    id.value = roleId;
    dialogTitle.value = "职级修改"
  } else {
    id.value = -1;
    dialogTitle.value = "职级添加"
  }
...
const handleDelete = async (id) => {
  ...
  const res = await requestUtil.del("level/action", ids)
...
const initLevelList=async ()=>{
  const res=await requestUtil.post("level/search",queryForm.value)
  tableData.value=res.data.levelList
